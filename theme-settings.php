<?php
// $Id: theme-settings.php,v 1.4 2010/03/11 18:48:02 atrasatti Exp $
...
function netspective_mobile_settings($saved_settings) {
  // Load color.inc to get the base color
 // require_once "color/color.inc";
  if (!isset($saved_settings['netspective_mobile_native']) || !(module_exists('mobileplugin'))) {
    $saved_settings['netspective_mobile_native'] = 1;
  }
 ...
 /* $form['recognition']['netspective_mobile_native'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Detect devices based on rules (default).'),
    ...
    '#default_value' => $saved_settings['netspective_mobile_native'],
    '#disabled'      => !(module_exists('mobileplugin')),
  );
  */
  // NOTE: If you change the list of colours here you should also update the function netspective_mobile_settings_css_filename() in template.php
